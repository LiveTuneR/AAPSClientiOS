...
Болюс доступен только через подписанный AndroidAPS 4 Client Control: master рассчитывает
дозу, возвращает текст подтверждения, а клиент отдельно подтверждает подготовленное действие
и проверяет подписанный прогресс доставки.
